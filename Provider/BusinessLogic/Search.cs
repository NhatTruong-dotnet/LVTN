...
namespace STU.LVTN.SERVER.Provider.BusinessLogic
{
    public class Search
    {
        ...
        {
            string[] searchParamsSplit = searchParams.Split(" ");
            List<int> idParents = new List<int>();
            List<int> idChilds = new List<int>();
            List<BaiDangEntities> resultSearchWithParams = new List<BaiDangEntities>();
            foreach (var splitItem in searchParamsSplit)
            {
                resultSearchWithParams.AddRange(_context.BaiDangs.Where(item => item.Mota.Contains(splitItem)).ToList());
                resultSearchWithParams.AddRange(_context.BaiDangs.Where(item => item.TieuDe.Contains(splitItem)).ToList());
            }
            
            foreach (var item in resultSearchWithParams)
            {
                if(!idParents.Contains(item.IdDanhMucCha))
                    idParents.Add(item.IdDanhMucCha);
                if (!idChilds.Contains((int)item.IdDanhMucCon))
                    idChilds.Add((int)item.IdDanhMucCon);
            }

            resultSearchWithParams = new List<BaiDangEntities>();
            List<BaiDangHomePageDTO> result = new List<BaiDangHomePageDTO>();
            int[] arrIdChilds = idChilds.ToArray();  
            int currentIdx = 0;
            foreach (var itemParent in idParents)
            {
                if (itemParent == 0)
                {
                    resultSearchWithParams.AddRange(_context.BaiDangs.Where(item => item.IdDanhMucCon == arrIdChilds[currentIdx]).ToList());
                    result = mapEntitiesToDTO(resultSearchWithParams);
                }
                else
                {
                    resultSearchWithParams.AddRange(_context.BaiDangs.Where(item => item.IdDanhMucCha == itemParent).ToList());
                    result = mapEntitiesToDTO(resultSearchWithParams);
                }
                currentIdx++;
            }
